...
{
  return std::list<TextTexture> { { std::to_string ( player_.get_hp () ), 100,
      10 }, { std::to_string ( get_score () ), 300, 10 }, { std::to_string (
      time_ / 60 ), 500, 10 } };
}
    ...
    Rectangle const& collision_target)
{
  ...
  switch (direction)
  {
    ...
    case Direction::kRight: // Moving object was to the right
      sprite.set_x (
          collision_target.get_x () + collision_target.get_width () );
      break;
